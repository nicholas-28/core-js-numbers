...
function roundToPowerOfTen(num, pow) {
  const round = Math.round(num / 10 ** pow) * 10 ** pow;
  return round;
}
...
function toNumber(value, def) {
  ...
  if (Number.isNaN(number)) return def;
  return number;
}
...
function getSumOfDigits(num) {
  const sumArr = num.toString().split('');
  let sum = 0;
...
function getNumberValue(number) {
  const primitive = number.valueOf();
  return primitive;
}
...
function isNumber(number) {
  ...
  const num = Number.isFinite(number);
  return num;
}
...
function getIntegerOnString(str, base) {
  const int = Number.parseInt(str, base);
  return int;
}
...
function isSafeInteger(number) {
  const safe = Number.isSafeInteger(number);
  return safe;
}
